...
import link.liycxc.utils.player.RotationUtil;
...
import link.liycxc.api.events.impl.*;
import link.liycxc.manages.component.impl.BadPacketsComponent;
import link.liycxc.manages.component.impl.InventoryDeSyncComponent;
...
import link.liycxc.utils.player.RayCastUtil;
...
public class KillAura extends Module {
    ...
    public void onPreMotion(EventPreMotion eventMotion) {
        ...
        if (target == null || mc.thePlayer.isDead || NekoCat.instance.moduleManager.getModule("Scaffold").getToggled()) {
            this.unblock(false);
            target = null;
        }
    }
    ...
    public void onUpdate(EventPreUpdate eventUpdate) {
        ...
        if (NekoCat.instance.moduleManager.getModule("Scaffold").getToggled() && !attackWhilstScaffolding.get()) {
            return;
        }
...
//                InventoryDeSyncComponent.setActive("/booster");
    ...
    public void onRender2D(EventRender2D event) {
        if (this.subTicks.get() && this.attack <= 5 && target != null && this.subTicksStopWatch.finished(10)) {
             ...
             * Getting targets and selecting the nearest one
             */
            targets = NekoCat.instance.targetManager.getTargets(range.get().doubleValue() + expandRange);
