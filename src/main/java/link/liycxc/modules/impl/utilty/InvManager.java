...
import link.liycxc.manages.component.impl.SelectorDetectionComponent;
...
public class InvManager extends Module {
    ...
    public void onPreMotionEvent(EventPreMotion event) {
        ...
        if (!NekoCat.instance.moduleManager.getModule("InvMove").getToggled() && !(mc.currentScreen instanceof GuiInventory)) {
            return;
        }
        ...
        if (block != -1 && block != this.blockSlot.get() - 1 && !NekoCat.instance.moduleManager.getModule("Scaffold").getToggled()) {
    ...
    private boolean canOpenInventory() {
        return NekoCat.instance.moduleManager.getModule("InvMove").getToggled() && !(mc.currentScreen instanceof GuiInventory);
    }
