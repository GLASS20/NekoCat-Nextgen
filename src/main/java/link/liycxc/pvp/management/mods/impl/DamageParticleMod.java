...
public class DamageParticleMod extends Mod {
	...
	public void onRender3D(EventRender3D event) {
		for (Particle particle : this.particles) {
			final double x = particle.location.getX() - (mc.getRenderManager()).getRenderPosX();
			final double y = particle.location.getY() - (mc.getRenderManager()).getRenderPosY();
			final double z = particle.location.getZ() - (mc.getRenderManager()).getRenderPosZ();
