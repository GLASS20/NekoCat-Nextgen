package link.liycxc.pvp.management.settings;
...
public class SettingsManager {
	...
	public Setting getSettingByClass(Class<?> modClass, String name){
		for(Setting set : getSettings()){
			if(set.getName().equalsIgnoreCase(name) && set.getParentMod().equals(NekoCat.instance.modManager.getModByClass((Class<Mod>) modClass))){
				return set;
			}
		}
		return null;
