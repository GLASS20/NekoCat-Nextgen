...
import link.liycxc.pvp.management.account.Account;
import link.liycxc.pvp.management.account.AccountType;
...
import link.liycxc.utils.SkinUtils;
...
public class GuiAccountManager extends GuiScreen{
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if(!showAddAccount) {
			...
			if(NekoCat.instance.accountManager.getAccounts().isEmpty()) {
			...
			for(Account a : NekoCat.instance.accountManager.getAccounts()) {
        ...
        if(delete) {
        	NekoCat.instance.accountManager.getAccounts().remove(deleteAccount);
			scrollY = 0;
        	delete = false;
        }
		...
		if(mouseButton == 0) {
			...
			if(showAddAccount) {
					...
					new Thread() {
						...
						public void run() {
							...
							try {
								...
								NekoCat.instance.accountManager.getAccounts().add(new Account(AccountType.MICROSOFT, acc.getProfile().getName(), acc.getProfile().getId(), acc.getRefreshToken()));
				...
				if(MouseUtils.isInside(mouseX, mouseY, x + 35, y + 91, 100, 20)) {
					...
					NekoCat.instance.accountManager.getAccounts().add(new Account(AccountType.CRACKED, username, "0", "0"));
					(mc).setSession(new Session(username, "0", "0", "legacy"));
					showAddAccount = false;
				}
				
				if(MouseUtils.isInside(mouseX, mouseY, (x + width) - 135, y + 91, 100, 20)) {
					if(!usernameField.getText().isEmpty()) {
						NekoCat.instance.accountManager.getAccounts().add(new Account(AccountType.CRACKED, usernameField.getText(), "0", "0"));
						(mc).setSession(new Session(usernameField.getText(), "0", "0", "legacy"));
						showAddAccount = false;
				...
				if(!showAddAccount) {
					if(MouseUtils.isInside(mouseX, mouseY, x + 10, y + offsetY + scrollAnimation.getValue(), width - 50, 35)) {
						...
						if(a.getAccountType().equals(AccountType.CRACKED)) {
							...
							NekoCat.instance.accountManager.setCurrentAccount(NekoCat.instance.accountManager.getAccountByUsername(a.getUsername()));
						}
					}
				}
