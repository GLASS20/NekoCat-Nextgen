...
import link.liycxc.pvp.management.colors.AccentColor;
...
public class SettingsCategory extends Category {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		int offsetX = 14;
		boolean darkMode = NekoCat.instance.settingsManager.getSettingByClass(ClientMod.class, "DarkMode").getValBoolean();
		int colorIndex = 1;
		...
		for(AccentColor c : NekoCat.instance.colorManager.getColors()) {
			...
			boolean selectedColor = NekoCat.instance.colorManager.getClientColor().equals(c);
	...
	public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if(mouseButton == 0) {
			if(MouseUtils.isInside(mouseX, mouseY, this.getX() + 106, this.getY() + 40, 80, 80)) {
				NekoCat.instance.settingsManager.getSettingByClass(ClientMod.class, "DarkMode").setValBoolean(true);
			}
			if(MouseUtils.isInside(mouseX, mouseY, this.getX() + 205, this.getY() + 40, 80, 80)) {
				NekoCat.instance.settingsManager.getSettingByClass(ClientMod.class, "DarkMode").setValBoolean(false);
			}
			for(AccentColor c : NekoCat.instance.colorManager.getColors()) {
				if(MouseUtils.isInside(mouseX, mouseY, this.getX() + 95 + offsetX + scrollAnimation.getValue(), this.getY() + 178 , 20, 20) && canToggle) {
					NekoCat.instance.colorManager.setClientColor(c);
				}
