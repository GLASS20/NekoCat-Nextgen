...
import link.liycxc.modules.impl.combat.KillAura;
...
public class TargetManager extends ConcurrentLinkedQueue<EntityLivingBase> {
    ...
    public void onTick(EventTick eventTick) {
        try {
            if (NekoCat.mc.thePlayer.ticksExisted % 150 == 0 || loadedEntitySize != NekoCat.mc.theWorld.loadedEntityList.size()) {
                this.updateTargets();
                loadedEntitySize = NekoCat.mc.theWorld.loadedEntityList.size();
            }
        } catch (Exception e) {
    ...
    public List<Entity> getTargets(final double range) {
        List<Entity> list = NekoCat.mc.theWorld.loadedEntityList.stream()
                .filter(entity -> isSelected(entity))
                .filter(entity -> NekoCat.mc.thePlayer.getDistanceToEntity(entity) < range)
                .sorted(Comparator.comparingDouble(entity -> NekoCat.mc.thePlayer.getDistanceSqToEntity(entity)))
                .collect(Collectors.toList());
        return list;
    }

    public boolean isSelected(final Entity entity) {
        boolean toNext = entity instanceof EntityLivingBase && entity.isEntityAlive() && entity != NekoCat.mc.thePlayer;
        if (toNext) {
    ...
    public int getPing(final EntityPlayer entityPlayer) {
        ...
        final NetworkPlayerInfo networkPlayerInfo = NekoCat.mc.getNetHandler().getPlayerInfo(entityPlayer.getUniqueID());
