package link.liycxc.utils.font;
...
public class MinecraftFontRenderer extends CFont {
    ...
    public void drawStringWithClientColor(String text, double x, double y, int opacity, boolean shadow) {
    	...
    	for(char textChar : text.toCharArray()) {
            ...
            if (!hasReachedSS) {
            	if(shadow) {
                	this.drawStringWithShadow(tmp, xTmp, y, link.liycxc.utils.color.ColorUtils.getClientColor(i, opacity).getRGB());
            	}else {
                	this.drawString(tmp, xTmp, y, link.liycxc.utils.color.ColorUtils.getClientColor(i, opacity).getRGB());
            	}
    ...
    public float drawString(String text, double x, double y, int color, boolean shadow, float kerning, boolean smooth) {
        ...
        link.liycxc.utils.color.ColorUtils.setColor(color);
        ...
        for (int index = 0; index < text.length(); index++) {
            ...
            if (character == '\u00a7') {
                ...
                if (colorIndex < 16) {
                    ...
                    link.liycxc.utils.color.ColorUtils.setColor(this.colorCode[colorIndex], alpha);
                } else {
                    switch (colorIndex) {
                        default:
                            link.liycxc.utils.color.ColorUtils.setColor(color);
