package link.liycxc.utils.color;

import link.liycxc.pvp.management.colors.AccentColor;
import link.liycxc.utils.animation.simple.SimpleAnimation;
import link.liycxc.utils.color.impl.ChristmasColorUtils;
import link.liycxc.utils.color.impl.HalloweenColorUtils;
...
public class ColorUtils {
	...
	private static Color getRawBackgroundColor(int id) {
		...
		boolean dark = NekoCat.instance.settingsManager.getSettingByClass(ClientMod.class, "DarkMode").getValBoolean();
	...
	public static Color getClientColor(int index, int alpha) {
		...
		for(AccentColor c : NekoCat.instance.colorManager.getColors()) {
			if(c.equals(NekoCat.instance.colorManager.getClientColor())) {
