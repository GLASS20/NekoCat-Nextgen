package link.liycxc.ui.clickgui.impl;
...
import link.liycxc.ui.clickgui.Category;
...
public class ConfigCategory extends Category {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		NekoCat.instance.moduleConfig.loadConfigs();
		
    	//Draw config list
    	for(File f : NekoCat.instance.moduleConfig.getConfigs()) {
        ...
        if(scroll != null && NekoCat.instance.moduleConfig.getConfigs().size() > 5) {
        	switch (scroll) {
        	case DOWN:
        		if(NekoCat.instance.moduleConfig.getScrollY() > -((NekoCat.instance.moduleConfig.getConfigs().size() - 5.5) * 35)) {
        			NekoCat.instance.moduleConfig.setScrollY(NekoCat.instance.moduleConfig.getScrollY() - 20);
        		}
        		
        		if(NekoCat.instance.moduleConfig.getScrollY() < -((NekoCat.instance.moduleConfig.getConfigs().size() - 6) * 35)) {
        			NekoCat.instance.moduleConfig.setScrollY(-((NekoCat.instance.moduleConfig.getConfigs().size() - 5.2) * 35));
        		}
        		break;
            case UP:
        		if(NekoCat.instance.moduleConfig.getScrollY() < -10) {
        			NekoCat.instance.moduleConfig.setScrollY(NekoCat.instance.moduleConfig.getScrollY() + 20);
        		}else {
            		if(NekoCat.instance.moduleConfig.getConfigs().size() > 5) {
            			NekoCat.instance.moduleConfig.setScrollY(0);
            		}
        		}
        		break;
        	}
        ...
        scrollAnimation.setAnimation((float) NekoCat.instance.moduleConfig.getScrollY(), 16);
	...
	public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
    	...
    	for(File f : NekoCat.instance.moduleConfig.getConfigs()) {
    		...
    		if(MouseUtils.isInside(mouseX, mouseY, (float) this.getX() + 95, (float) (this.getY() + offsetY + scrollAnimation.getValue()), 165, 26) && mouseButton == 0 && canToggle) {
    			...
    			NekoCat.instance.moduleConfig.load(f);
		...
		if(MouseUtils.isInside(mouseX, mouseY, this.getX() + 235, this.getY() + 205, 60, 21) && mouseButton == 0) {
			...
			NekoCat.instance.moduleConfig.save(new File(NekoCat.instance.fileManager.getNConfigDir(), configName + ".nekocat"));
